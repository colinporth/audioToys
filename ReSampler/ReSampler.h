...
namespace ReSampler {
  ...
  template<typename FloatType> SndfileHandle* getTempFile(int inputFileFormat, int nChannels, const ConversionInfo& ci, std::string& tmpFilename);
  ...
  //{{{
  template <typename InputIterator> int runCommand(InputIterator first, InputIterator last)
  {
  ...
  //}}}
  //{{{
  template <typename ContainerType> int runCommand(const ContainerType& args)
  {
