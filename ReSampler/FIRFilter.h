...
namespace ReSampler {
  //{{{
  template <typename FloatType> class FIRFilter {
  public:
    //{{{
  ...
  private:
    ...
    //}}}
       ...
       // Horizontal add function (sums 4 doubles into single double)
         ...
         const __m128d x128 = _mm_add_pd (_mm256_extractf128_pd(x, 1), _mm256_castpd256_pd128(x));
  ...
  // Specializations for doubles:
  #if defined(USE_AVX)
    //{{{
    template <> double FIRFilter<double>::get() {
    ...
    //{{{
    template <> inline double FIRFilter<double>::get() {
  ...
  // in which case, Alpha def= Beta / pi
    ...
    if (Length < 1)
      ...
      for (int n = 0; n < Length; ++n) {
        filter[n] *= I0q(Beta * sqrtq(1.0Q - powq((2.0Q * n / (Length - 1) - 1), 2.0Q))) / I0q(Beta);
        }

    #else
      for (int n = 0; n < Length; ++n) {
        filter[n] *= I0(Beta * sqrt(1.0 - pow((2.0 * n / (Length - 1) - 1), 2.0))) / I0(Beta);
        }
  ...
  //}}}
  //{{{
  template<typename FloatType> void makeMinPhase(FloatType* pFIRcoeffs, size_t length) {
  // makeMinPhase() : transform linear-phase FIR filter coefficients into minimum-phase (in-place version)
    ...
    // take the reversed array of the real parts of the ifft of e to the power of the Analytic Signal of
    //   the real parts of the log of the dynamic-ranged limited version of the fft of the original filter
  ...
  //}}}
  //{{{
  template<typename FloatType> void dumpFFT (FloatType* data, size_t length) {
