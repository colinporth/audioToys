...
namespace ReSampler {
  ...
  //{{{
  template <typename FloatType> std::vector<FloatType> makeFilterCoefficients (const ConversionInfo& ci, Fraction fraction) {
    ...
    int sidelobeAtten = ((fraction.numerator == 1) || (fraction.denominator == 1)) ? 195 : 160;
  ...
  //}}}
  //{{{
  template <typename FloatType> class ResamplingStage {
  public:
    //{{{
  ...
  private:
    ...
    void interpolateAndDecimate (FloatType* outBuffer, size_t& outBufferSize,
                                 const FloatType* inBuffer, const size_t& inBufferSize) {
      size_t o = 0;
      int localm = m;
      for (size_t i = 0; i < inBufferSize; ++i) {
        for(int l = 0; l < L; ++l) {
  ...
  //}}}
  //{{{
  template <typename FloatType> class Converter {
  public:
    //{{{
