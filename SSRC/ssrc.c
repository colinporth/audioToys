...
double downsample (FILE *fpi,FILE *fpo,int nch,int bps,int dbps,int64_t sfrq,int64_t dfrq,double gain,unsigned int chunklen,int twopass,int dither)
{
...

  {
    int n1b2 = n1b/2;
    int rp;        // inbufのfs1
    int rps;       // rpを(fs1/sfrq=osf)
    int rp2;       // buf2のfs2
    int ds;        // 次にdisposeするsfrq
    int nsmplwrt1 = 0; // 実際にファイルからinbuf
                   // stage2 filter
    int nsmplwrt2 = 0; // 実際にファイルからinbuf
                   // stage2 filter
    int s2p;       // stage1 filterから出力されたサンプルの数をn1y*osf
    int init,ending;
    int osc;
    REAL *bp,*bp_backup; // rp2から計算される．buf2
...
int extract_int (uint8_t *buf) {
    ...
    return *(int *)buf;
  #else
    return ((int)buf[0]) | (((int)buf[1]) << 8) | (((int)buf[2]) << 16) | (((int)((int8_t *)buf)[3]) << 24);
  #endif
}
//}}}
//{{{
...
int main (int argc, char **argv)
{
  ...
  if (!quiet) {
    //{{{
    const char *ptype[] = { "rectangular", "triangular", "gaussian", "two-level" };
